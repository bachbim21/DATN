...
import { EquipmentGroupFullInfoDto, GetEquipmentGroupsQueryParam } from 'types/equipmentGroup.type';
...
import { ModalCreateEquipmentGroup } from './ModalCreateEquipmentGroup';
import { ModalUpdateEquipmentGroup } from './ModalUpdateEquipmentGroup';
...
const EquipmentGroup = () => {
  ...
  const [supplies, setSupplies] = useState<EquipmentGroupFullInfoDto[]>([]);
  ...
  const [getEquipmentGroupQueryParam, setGetEquipmentGroupsQueryParam] = useState<GetEquipmentGroupsQueryParam>({});
  const keywordSearch = useDebounce(getEquipmentGroupQueryParam.keyword as string, 500);
  ...
  const [showCreateEquipmentGroupModal, setShowCreateEquipmentGroupModal] = useState<boolean>(false);
  const [showUpdateEquipmentGroupModal, setShowUpdateEquipmentGroupModal] = useState<boolean>(false);
  const [updateEquipmentGroupModalData, setUpdateEquipmentGroupModalData] = useState<EquipmentGroupFullInfoDto>({});
  const columns: any = [
    {
      ...
      title: 'Tác vụ', key: 'action', show: true, render: (item: EquipmentGroupFullInfoDto) => (<Menu className='flex flex-row items-center'>
        {hasAuthority(Authority.EQUIPMENT_GROUP_UPDATE) && <Menu.Item>
          <Tooltip title='Cập nhật thông tin nhóm thiết bị'>
            <EditFilled
              onClick={() => {
                setUpdateEquipmentGroupModalData(item);
                setShowUpdateEquipmentGroupModal(true);
              }} />
          </Tooltip>
        </Menu.Item>}
        {hasAuthority(Authority.EQUIPMENT_DELETE) && <Menu.Item>
          <Tooltip title='Xóa'>
            <Popconfirm
              ...
              onConfirm={() => handleDeleteEquipmentGroup(item.id as number)}
              okText='Xóa'
              cancelText='Hủy'
            >
              <DeleteFilled />
  ...
  const onChangeQueryParams = (key: string, value: string | moment.Moment[] | undefined | number | PageableRequest) => {
    ...
    let getEquipmentGroupQueryParamClone: GetEquipmentGroupsQueryParam = getEquipmentGroupQueryParam;
    if (key === '') { //if key is empty, it means that the component should be updated
      setComponentShouldUpdate(true);
    }
    if (key === 'keyword') {
      getEquipmentGroupQueryParamClone = { ...getEquipmentGroupQueryParam, keyword: value as string };
    }
    if (key === 'pageable') {
      pagebaleClone = { ...pageableRequest, page: (value as PageableRequest).page, size: (value as PageableRequest).size };
    }
    setGetEquipmentGroupsQueryParam(getEquipmentGroupQueryParamClone);
    setPageableRequest(pagebaleClone);
    const url = createUrlWithQueryString(location.pathname, getEquipmentGroupQueryParamClone, pagebaleClone);
    setQueryString(url);
    navigate(url);
  };
  ...
  const handleDeleteEquipmentGroup = (id: number) => {
    equipmentGroupApi.deleteEquipmentGroup(id).then(() => {
  ...
  const searchListOfEquipmentGroup = (queryParams: GetEquipmentGroupsQueryParam, pageableRequest: PageableRequest) => {
    equipmentGroupApi.getEquipmentGroups(queryParams, pageableRequest).then((res) => {
      if (res.data.success) {
  ...
  useEffect(() => {
    ...
    setGetEquipmentGroupsQueryParam({
      keyword: query.keyword || '',
    });
    setPageableRequest({
      page: Number(query.page) || 0, size: Number(query.size) || 20,
    });
    searchListOfEquipmentGroup(getEquipmentGroupQueryParam, pageableRequest);
    // setLoading(false);
  ...
  return (<div>
    <div className='flex-between-center'>
      ...
      <div className='flex flex-row gap-6'>
        <Button
          ...
          onClick={() => setShowCreateEquipmentGroupModal(true)}
        >
          <PlusCircleFilled />
    ...
    <ModalCreateEquipmentGroup
      showCreateEquipmentGroupModal={showCreateEquipmentGroupModal}
      hideCreateEquipmentGroupModal={() => setShowCreateEquipmentGroupModal(false)}
      callback={() => {
        onChangeQueryParams('', '');
      }} />
    <ModalUpdateEquipmentGroup
      showUpdateEquipmentGroupModal={showUpdateEquipmentGroupModal}
      hideUpdateEquipmentGroupModal={() => setShowUpdateEquipmentGroupModal(false)}
      callback={() => {
        onChangeQueryParams('', '');
      }}
      updateEquipmentGroupModalData={updateEquipmentGroupModalData}
    />
  </div>);
};
